...
Page({
  ...
  onSelectType(e) {
    const { name, met } = e.currentTarget.dataset
    this.setData({ selectedType: name, selectedMet: met })
    this.calcCalories()
  },
  ...
  calcCalories() {
    ...
    // 卡路里 = MET x 体重(kg) x 时长(h)
    const calories = Math.round(selectedMet * weight * (duration / 60))
    this.setData({ calories })
  },

  async onSubmit() {
    const { selectedType, duration, calories, selectedMet } = this.data
    ...
    try {
      ...
      this.setData({ selectedType: '', selectedMet: 0, duration: 0, customDuration: '', calories: 0 })
      this.loadTodayRecords()
      this.calcStreak()
    } catch (err) {
